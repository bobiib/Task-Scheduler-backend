...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    CHECK (
        (schedule_type = 'date' AND schedule_date IS NOT NULL)
        OR (schedule_type = 'week'
            AND schedule_year IS NOT NULL
            AND schedule_month IS NOT NULL
            AND schedule_week_of_month IS NOT NULL)
        OR (schedule_type = 'weekday_in_month'
            AND schedule_year IS NOT NULL
            AND schedule_month IS NOT NULL
            AND schedule_week_of_month IS NOT NULL
...
CREATE TABLE IF NOT EXISTS task_recurrences (
    ...
    repeat_type ENUM('daily', 'weekly', 'monthly_day', 'monthly_weekday', 'yearly') NOT NULL,
    ...
    CHECK (
        repeat_type IN ('daily', 'weekly')
        OR (repeat_type = 'monthly_day' AND day_of_month IS NOT NULL)
        OR (repeat_type = 'monthly_weekday'
            AND week_of_month IS NOT NULL
            AND weekday IS NOT NULL)
        OR (repeat_type = 'yearly'
...
CREATE TABLE IF NOT EXISTS task_reminders (
    ...
    FOREIGN KEY (task_id) REFERENCES tasks(id) ON DELETE CASCADE,
    CHECK (amount > 0),
    UNIQUE KEY uq_task_reminder (task_id, amount, unit),
...
CREATE TABLE IF NOT EXISTS reminder_deliveries (
    ...
    FOREIGN KEY (reminder_id, task_id)
        REFERENCES task_reminders(id, task_id) ON DELETE CASCADE,
    FOREIGN KEY (task_instance_id, task_id)
        REFERENCES task_instances(id, task_id) ON DELETE CASCADE,
    FOREIGN KEY (recipient_user_id) REFERENCES users(id) ON DELETE CASCADE,

    UNIQUE KEY uq_delivery (reminder_id, task_instance_id, recipient_user_id),
    CHECK (
        (status = 'sent' AND sent_at IS NOT NULL)
        OR (status IN ('pending', 'failed') AND sent_at IS NULL)
    ),
    INDEX idx_reminder_delivery_queue (status, scheduled_for),
    INDEX idx_reminder_recipient (recipient_user_id)
);
